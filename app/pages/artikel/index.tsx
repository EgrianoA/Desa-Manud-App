   ...
   useEffect(() => {
      if (dataSource) {
         ...
         if (dataSource.data.length) {
            return setArticleData(uniqBy([...articleData, ...dataSource.data], '_id'))
         }
      }
   }, [dataSource])

   const refresh = useCallback(() => { console.log('ON REFRESH', values) }, [])
            ...
            <Row>
               ...
               >
                  {articleData.map((article) => (
                     <Card
                        title={`${dayjs(article.createdAt).format('DD/MM/YYYY HH:mm')} - ${article.title}`}
                        bordered={false}
